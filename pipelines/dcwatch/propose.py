...
def propose(corpus_dir: Path, records: list[dict]) -> list[dict]:
    ...
    for path in sorted(corpus_dir.glob("*.json")):
        ...
        hits = []
        for r in by_commune.get(_commune_key(identity["municipality"]), []):
            row_key = normalize_name(f"{r['name']} {r['operator']}")
            row_tokens = set(_OPERATOR_TOKENS.findall(row_key))
            if (dc_tokens & row_tokens) or _name_overlap(dc_key, row_key):
                hits.append(r)
        powers = {round(r["power_mw"], 1) for r in hits}
        if len(powers) == 1:
            r = hits[0]
            out.append({
            ...
            out.append({"dc_id": dc["id"], "decision": "ambiguous",
                        "candidates": [{"name": r["name"], "power_mw": r["power_mw"]} for r in hits]})
        else:
